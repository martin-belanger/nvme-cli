...
struct active_ctrl {
	...
	bool force_persistent_logged; // the override notice fired once already
				  ...
				  const struct libnvmf_tid *dc_tid)
{
	...
	if (e && e->is_dc && !epcsd) {
		if (e->force_persistent) {
			if (!e->force_persistent_logged) {
				disc_info("%s - EPCSD=0, but persistent=force: staying connected",
					  libnvmf_tid_str(dc_tid));
				e->force_persistent_logged = true;
			}
		} else {
			epcsd_park(e);
		}
