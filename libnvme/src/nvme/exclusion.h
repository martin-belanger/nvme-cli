 ...
 * controllers that a local administrator has explicitly excluded.  It is
 * stored in /etc/nvme/exclusions/ as one or more .conf files, each holding
 * any number of "exclusion = key=val;key=val" entries.
 *
 ...
 *            validated; invalid entries cause -EINVAL and no write)
