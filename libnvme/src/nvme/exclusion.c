 ...
 * Format:  one "exclusion = key=val;key=val" line per entry; # lines are comments.
 */
					      ...
					      const char *name, const char *entry)
{
	...
	while (fgets(line, sizeof(line), fin)) {
		...
		/* Parse a scratch copy; libnvmf_trim() mutates in place and would
		 * otherwise clobber the trailing newline in "line" before
		 * it gets passed through to fout.
		 */
		strncpy(parsebuf, line, sizeof(parsebuf) - 1);
		parsebuf[sizeof(parsebuf) - 1] = '\0';
		s = libnvmf_trim(parsebuf);
 ...
 * caller to have pre-checked the buffer.
 */
static int validate_conf_buf(struct libnvme_global_ctx *ctx, const char *text)
{
	...
	for (line = strtok_r(copy, "\n", &save); line;
	     line = strtok_r(NULL, "\n", &save)) {
		char *s = libnvmf_trim(line), *eq, *key, *val;
