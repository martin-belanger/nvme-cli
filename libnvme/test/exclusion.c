...
static bool test_stale_rejected(struct libnvme_global_ctx *ctx)
{
	...
	ret = libnvmf_exclusion_write(ctx, "list", "exclusion = transport=tcp\n", ver);
	if (ret) {
	...
	ret = libnvmf_exclusion_write(ctx, "list", "exclusion = transport=fc\n", ver);
	if (ret != -ESTALE) {
...
static bool test_invalid_entry_rejected(struct libnvme_global_ctx *ctx)
{
	...
	ret = libnvmf_exclusion_write(ctx, "list", "exclusion = boguskey=x\n", ver);
	if (ret != -EINVAL) {
...
static bool test_missing_then_create(struct libnvme_global_ctx *ctx)
{
	...
	ret = libnvmf_exclusion_write(ctx, "fresh", "exclusion = transport=tcp\n", 0);
